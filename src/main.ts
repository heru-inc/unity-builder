...
// vitest's module re-loading (which changed in vitest 4). The action entry
// points (dist/main.js / dist/post.js) call the exports re-exported by index.ts.
...
async function runLocalBuild(
  ...
  // Runner context (temp path + action id) lets Docker.run write a --cidfile so
  // the post step can return the floating license if the job is cancelled.
